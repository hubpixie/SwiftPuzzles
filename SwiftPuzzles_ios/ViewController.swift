...
class ViewController: UIViewController {
    ...
    func adjustViewLayout() {
        ...
        if UIScreen.main.nativeBounds.height >= 2436  {
            ...
            self.defSafeAreaXOffset = 0.0
            self.defSafeAreaYOffset = 0.0
        }
        self.oCollectionView.frame = CGRect(x: frame2.origin.x, y: frame2.origin.y, width: frame2.size.width + self.defSafeAreaXOffset, height: frame2.size.height + (bounds.size.height - frame2.size.height - self.defSafeAreaYOffset))
    }
}
