/*
	...
	- String to BigInt conversion is 2x faster.
	- BigInt to String also performs 50% better.

	———— v1.5 ——————————————————————————————————————————————————————————————————
	- Updated for full Swift 3 compatibility.
	- Various optimizations:
	- Multiplication is about 2x faster.
	- BigInt to String conversion is more than 3x faster.
	- String to BigInt conversion is more than 2x faster.
	...
	———— Evolution —————————————————————————————————————————————————————————————
	Planned features of BigInt v2.0:
...
/*
	...
	fits into an unsigned 64 bit number. Digits are required for printing BigInt
	...
	Hashable
{
	...
	*/
	...
	{
		precondition(limbs != [], "BigInt can't be initialized with limbs == []")
	...
	/// Use a string to create a new BigInt number.
	...
	/// Returns BigInt value as an integer, if possible.
...
/******\
/*******\
//MARK: - Public operators for shifting BigInts
\*******/
\******/
\*****/
\****/
...
public func >>(lhs: BigInt, rhs: Int) -> BigInt
{
	...
	return BigInt(sign: lhs.sign, limbs: lhs.limbs >>° rhs)
}
...
public func abs(_ lhs: BigInt) -> BigInt
{
	return BigInt(
		sign: false,
		limbs: lhs.limbs
...
private func addBigInt(_ a: Limbs, _ b: Limbs) -> Limbs
{
	var res:    Limbs = a ^^° b
	var ovf:    Limbs = (a &° b) <<° 1
	var ovfNew: Limbs = [0]
...
private func addLimbs(_ lhs: inout Limbs, _ rhs: Limbs, rhsPaddingZeroLimbs pz: Int)
{
	...
	while i < min
	{
		if ovfl
		{
//            (newLimb, ovfl) = Limb.addingReportingOverflow(lhs[i], rhs[i &- pz])
...
/******\
/*******\
//MARK: - Public operators for adding BigInts
\*******/
\******/
\*****/
\****/
...
public func +(lhs: Int, rhs: BigInt) -> BigInt { return BigInt(lhs) + rhs }
public func +(lhs: BigInt, rhs: Int) -> BigInt { return lhs + BigInt(rhs) }

public func +=(lhs: inout Int, rhs: BigInt) { lhs += (BigInt(lhs) + rhs).toInt()! }
public func +=(lhs: inout BigInt, rhs: Int) { lhs +=  BigInt(rhs)                 }
...
/******\
/*******\
//MARK: - Public operators for subtracting BigInt's
\*******/
\******/
\*****/
\****/
...
public prefix func -(n: BigInt) -> BigInt
{
	var n = n
	n.negate()
	return n
...
public func -=(lhs: inout BigInt, rhs: BigInt) { lhs +=             -rhs           }
public func -=(lhs: inout Int, rhs: BigInt)  { lhs  = (BigInt(lhs) - rhs).toInt()! }
public func -=(lhs: inout BigInt, rhs: Int)  { lhs -=         BigInt(rhs)          }
...
/******\
/*******\
//MARK: - Public operators for multiplying BigInt's
\*******/
\******/
\*****/
\****/
...
public func *(lhs: BigInt, rhs: BigInt) -> BigInt
{
	...
	return BigInt(sign: sign, limbs: lhs.limbs *° rhs.limbs)
}

public func *(lhs: Int, rhs: BigInt) -> BigInt { return BigInt(lhs) * rhs }
public func *(lhs: BigInt, rhs: Int) -> BigInt { return lhs * BigInt(rhs) }

public func *=(lhs: inout BigInt, rhs:BigInt) { lhs =       lhs  * rhs          }
public func *=(lhs: inout Int, rhs: BigInt) { lhs = (BigInt(lhs) * rhs).toInt()! }
public func *=(lhs: inout BigInt, rhs: Int) { lhs =       lhs  * BigInt(rhs)    }
...
/******\
/*******\
//MARK: - Public operators for exponentiation of BigInt's
\*******/
\******/
\*****/
\****/
...
public func ^(lhs: BigInt, rhs: Int) -> BigInt
{
	precondition(rhs >= 0, "BigInts can't be exponentiated with exponents < 0")

	return BigInt(sign: lhs.sign && ((rhs & 1) == 1), limbs: lhs.limbs ^° rhs)
}
...
/******\
/*******\
//MARK: - Public operators for modulo calculations with BigInt's
\*******/
\******/
\*****/
\****/
...
/******\
/*******\
//MARK: - Public operators for division with BigInt's
\*******/
\******/
\*****/
\****/
...
/******\
/*******\
//MARK: - Public operators for comparing BigInt's
\*******/
\******/
\*****/
\****/
...
public func ==(lhs: Int, rhs: BigInt) -> Bool { return BigInt(lhs) == rhs }
public func ==(lhs: BigInt, rhs: Int) -> Bool { return lhs == BigInt(rhs) }
...
public func fact(_ n: Int) -> BigInt
{
	return BigInt(limbs: factRec(1, n))
}
...
public func steinGcd(_ a: BigInt, _ b: BigInt) -> BigInt
{
...
public func gcd(_ a: BigInt, _ b: BigInt) -> BigInt
{
	...
	return BigInt(sign: a.sign && limbRes != [0], limbs: limbRes)
}
...
public func fib(_ n:Int) -> BigInt
{
	var a: Limbs = [0]
	var b: Limbs = [1]
...
{
	return BigInt(limbs: factRec(n - k, n))
}

public func combinations(_ n: Int, _ k: Int) -> BigInt
{
	return BigInt(limbs: factRec(n - k, n) /° factRec(1, k))
}

public func randomBigInt(bits n: Int) -> BigInt
{
	let limbs = n >> 6
	let singleBits = n % 64
	...
	Hashable
{
	...
	*/
	...
	init(_ numerator: BigInt, over denominator: BigInt)
	{
		self.init(
	...
	{
		self.init(BigInt(numerator), over: BigInt(denominator))
	}
